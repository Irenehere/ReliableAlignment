...
def train_model(model, df_train, df_val, data_collator):
    ...
    def preprocess_function(example):
        return strong_tokenizer(example["new_input"],truncation=True,max_length=1600)
    ...
    ds_tokenized_train = ds_tokenized_train.remove_columns(["new_input"])
    ds_tokenized_val = ds_tokenized_val.remove_columns(["new_input"])
...
def prepare_validation_set_ETHICS(weak_model_name): 
    df_val = pd.read_csv('cm_validation_with_entropy_from_{}.csv'.format(weak_model_name))
    ...
    for i in tqdm(range(0,len(df_val),2)):
        batch = df_val.loc[i:i+1]
        if batch['model_pred'].isnull().any():
            continue
        example_input = batch['input'].unique()
        assert len(example_input)==1
        answers = batch['model_pred'].value_counts().to_dict()
        answers_ = get_answer_score(answers,temperature=1)
        for i in batch.index:
            batch.at[i,'answer_score'] = answers_[batch.at[i,'model_pred']]

    df_val = df_val[df_val['answer_score']!=""]
    df_val["new_input"] = df_val['prompt']+" "+df_val['model_output']
    df_val = df_val.drop(columns=['input','is_short','label','prompt','model_output','model_pred','entropy'])
    return df_val
...
if __name__ == "__main__":
    ...
    # load data
    df_val = prepare_validation_set_ETHICS(args.weak_model_name)
    print("Loaded validation set: ", df_val.head())

    # sample data
    if args.do_sampling:
        df_val_new = df_val.sample(n = int(len(df_val)/2)).reset_index(drop=True)
    else:
